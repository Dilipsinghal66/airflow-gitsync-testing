...
def broadcast_days_inactive():

    process_broadcast_days_inactive_disable = int(Variable.get
                                                ("process_broadcast_days_"
                                                 "inactive_disable", '0'))
    if process_broadcast_days_inactive_disable == 1:
        return

    message = str(Variable.get("broadcast_days_inactive_msg", ''))

    delta = int(Variable.get("broadcast_days_inactive_delta", '2'))
    start_date = datetime.datetime.today() - datetime.timedelta(days=delta)
    _filter = {
        "lastActivity": {"$lte": start_date}
    }
