...
def get_patient_ids():
    try:
        ...
        filter_active_patient_query = "select id from patient_profile where status=4 and new_chat=1 and id not " \
...
def broadcast_active_minus_preg():
    patientIds = get_patient_ids()
    print(patientIds)
    process_broadcast_active = int(Variable.get("broadcast_active_minus_preg", '0'))
