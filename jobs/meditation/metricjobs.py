...
def create_meditation_metrics():
    try:
        ...
        cursor.execute(
            "select count(distinct patientId) from zylaapi.meditationLogs where forDate < CURDATE() and forDate > DATE_SUB(CURDATE(), INTERVAL 2 DAY) and startEnd = 1")
        totalcount = cursor.fetchone()[0]
        # print(totalcount)
        numberofPage = int(totalcount / PAGE_SIZE) + 1
        print(numberofPage)
        for i in range(numberofPage):
            patientIdSqlQuerry = "select distinct patientId from zylaapi.meditationLogs where forDate < CURDATE() and forDate > DATE_SUB(CURDATE(), INTERVAL 2 DAY) and startEnd = 1 LIMIT " + str(
