...
def create_reportings_func():
    try:
        ...
        for i in range(numberofPage):
            patientIdSqlQuerry = "select id from zylaapi.patient_profile LIMIT " + str(  # noqa E303
            ...
            for patientid in patientIdList:
                checkSqlQuery = "select id from zylaapi.reportings where Date(due_date) = curdate() and patient_id = " + str(  # noqa E303
                    patientid)
                no_of_rows = cursor.execute(checkSqlQuery)
                if no_of_rows == 0:
                    insertSqlQuery = "INSERT INTO zylaapi.reportings (patient_id, due_date)  VALUES (" + str(  # noqa E303
                        patientid) + ", CURRENT_TIMESTAMP())"
