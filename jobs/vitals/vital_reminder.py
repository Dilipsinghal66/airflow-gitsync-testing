...
def send_vital_reminder_func():
    try:
        ...
        cursor.execute("select distinct(id) from zylaapi.auth where phoneno in (select phoneno from zylaapi.patient_profile where id in (select distinct(patientId) from zylaapi.patientTestReadings where forDate=CURDATE() and isRecommended = 1))")    # noqa E303
        patientIdList = []
        ...
        recParamMsgSqlQuery = "select name from zylaapi.params where id in (" + recParam + ")"    # noqa E303
